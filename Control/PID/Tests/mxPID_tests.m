classdef mxPID_tests < matlab.unittest.TestCase
    % Unit Tests for the mxPID MEX Interface to the C++ PID controller
    % J.Currie Apr 2023
    ...
    methods(Test)
        ...
        function CheckInit(testCase)
            % No params
            clear mxPID;
            testCase.verifyError(@() mxPID('init'),'GNCToolsMEX:Error');

            % Check Inf/NaN for gains
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(Inf, 0, 0, 0.1)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(NaN, 0, 0, 0.1)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, Inf, 0, 0.1)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, NaN, 0, 0.1)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 1, Inf, 0.1)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 1, NaN, 0.1)));
            
            % Check 0/I/ID/D only
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(0, 0, 0, 0.1)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(0, 1, 0, 0.1)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(0, 0, 1, 0.1)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(0, 1, 1, 0.1)));

            % Check Ts
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 0, 0, 0)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 0, 0, -1)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 0, 0, Inf)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 0, 0, NaN)));

            % Check Tf
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 0, 0, 0.1, 0.1))); % no D gain
            testCase.verifyEqual(int8(0), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, 0))); % disabled OK
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, -1)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, Inf)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, NaN)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, 0.05)));
            testCase.verifyEqual(int8(0), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, 0.051)));

            % Check b, c
            testCase.verifyEqual(int8(0), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, 0.1, 0)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, 0.1, -0.1)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, 0.1, Inf)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, 0.1, NaN)));
            testCase.verifyEqual(int8(0), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, 0.1, 0, 0)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, 0.1, 0, -0.1)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, 0.1, 0, Inf)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, 0.1, 0, NaN)));

            % Check uMin, uMax
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, 0.1, 0, 0, NaN)));
            testCase.verifyEqual(int8(-1), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, 0.1, 0, 0, 0, NaN)));
        ...
        function CheckUpdate(testCase)
            % No init
            clear mxPID;
            testCase.verifyError(@() mxPID('update',1,1),'GNCToolsMEX:Error'); 

            % Do init
            testCase.verifyEqual(int8(0), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1)));

            % Missing args
            testCase.verifyError(@() mxPID('update'),'GNCToolsMEX:Error');
            testCase.verifyError(@() mxPID('update', 1),'GNCToolsMEX:Error');
            
            % Get current control
            [~,status] = mxPID('update', 1, 0);
            testCase.verifyEqual(int8(0), status);

            % Inf/NaN setpoint
            [~,status] = mxPID('update', Inf, 0);
            testCase.verifyEqual(int8(-1), status);
             [~,status] = mxPID('update', NaN, 0);
            testCase.verifyEqual(int8(-1), status);

            % Inf/NaN measurement
            [~,status] = mxPID('update', 1, Inf);
            testCase.verifyEqual(int8(-1), status);
             [~,status] = mxPID('update', 1, NaN);
            testCase.verifyEqual(int8(-1), status);                
        end

        function CheckReset(testCase)
            % No init OK
            testCase.verifyEqual(int8(0), mxPID('reset'));

            % Do init and couple updates, then reset and check
            testCase.verifyEqual(int8(0), mxPID('init',testCase.makeTestControllerParams(1, 1, 1, 0.1, 0.1)));
            [u0,status] = mxPID('update', 1, 0);
            testCase.verifyEqual(int8(0), status);
            [u1,status] = mxPID('update', 1, 0);
            testCase.verifyEqual(int8(0), status);
            testCase.verifyNotEqual(u0,u1);

            testCase.verifyEqual(int8(0), mxPID('reset'));
            [u2,status] = mxPID('update', 1, 0);
            testCase.verifyEqual(int8(0), status);
        ...
        function PIDSetpointRamp(testCase)            
            ...
            % Do init
            testCase.verifyEqual(int8(0), mxPID('init',testParams));
            
            % Do all updates one by one ramping up
            [~,status,rOut] = mxPID('update', r, 0);
            testCase.verifyEqual(int8(0), status);
